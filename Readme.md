# GPGithub
GPGithub is not only a shell script that automates the process of creating new
...
It is meant to bring GPG to everybody as easy as possible.
...
## GPG quick start guide
GPGithub guides you through 5 simple steps to get your software project ready
...
Key expires at Mon 30 Dec 2019 11:40:44 PM CET
...
```
gpg: key 3805EF58CCFDA6AE marked as ultimately trusted
gpg: directory '/home/arch/.gnupg/openpgp-revocs.d' created
gpg: revocation certificate stored as '/home/arch/.gnupg/openpgp-revocs.d/9DCA8286C993F474A160CE8A3805EF58CCFDA6AE.rev'
public and secret key created and signed.

pub   ed25519 2016-12-04 [SC] [expires: 2017-12-04]
      9DCA8286C993F474A160CE8A3805EF58CCFDA6AE
      9DCA8286C993F474A160CE8A3805EF58CCFDA6AE
uid                      testname <testmail@test.test>
sub   cv25519 2016-12-04 [E] [expires: 2017-12-04]
```

The generated key has the fingerprint `9DCA8286C993F474A160CE8A3805EF58CCFDA6AE`
...
```bash
gpg --keyserver hkps://hkps.pool.sks-keyservers.net --send-keys 9DCA8286C993F474A160CE8A3805EF58CCFDA6AE
```

Now the user can get your key by requesting the fingerprint from the keyserver:
```bash
gpg --keyserver hkps://hkps.pool.sks-keyservers.net --recv-keys 9DCA8286C993F474A160CE8A3805EF58CCFDA6AE
```
...
```
git archive --format=tar --prefix "gpgithub-1.0.0" 1.0.0 | xz -9 > gpgithub-1.0.0.tar.xz"
```
...
```bash
sha512 gpgithub-1.0.0.tar.xz > gpgithub-1.0.0.tar.xz.sha512
```

#### 4.3 Sign the sources
Type the filename of the tarball that you want to sign and then run:
```bash
gpg --armor --detach-sign gpgithub-1.0.0.tar.xz
```
...
This gives you a file called `gpgithub-1.0.0.tar.xz.asc` which is the GPG
...
[GPGithub](https://github.com/NicoHood/gpgithub) is meant to bring GPG to everybody as easy as possible. It automates/simplifies the signing process and gives instructions/help for manual signing.
...
```
1.0.0 (xx.xx.201x)
* Merged all scripts into gpgithub.sh
